...
for freq in "${freq_list[@]}"; do
    ...
    mkdir -p performance_streaming_output
    cd performance_streaming_output
    ...
    for (( i=1; i<=5; i++ )); do
        ...
        # Run the simulation
        gmx mdrun -v -deffnm ${run_folder}/run -ntmpi 1 -ntomp ${n_cpus} -gpu_id 0 -imdwait -imdport 8888 >& "${run_folder}/production.out" &
        sleep 20
        python3 ${input_folder}/IMDv3-client-output.py "${run_folder}" &
        wait
    done
